...
DWORD WINAPI main(PVOID base)
{
	...
	std::string folder1, folder2, folder3;
	...
	{
		folder1 = std::string(path) + crypt_str("\\Alphen\\");
		folder2 = std::string(path) + crypt_str("\\Alphen\\configs\\");
		folder3 = std::string(path) + crypt_str("\\Alphen\\scripts\\");
	}

	CreateDirectory(folder1.c_str(), NULL);
        CreateDirectory(folder2.c_str(), NULL);
	CreateDirectory(folder3.c_str(), NULL);
