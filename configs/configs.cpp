...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.ragebot.optimizer, false, crypt_str("Ragebot.optimizer"));
	...
	setup_item(&g_cfg.player.show_resolved, false, crypt_str("Player.show_resolved"));
	setup_item(&g_cfg.player.esp_prewiev, false, crypt_str("Player.esp_prewiev"));
	...
	for (auto i = 0; i < 3; i++)
	{
		...
		setup_item(&g_cfg.player.type[i].flags, 9, std::to_string(i) + crypt_str("Player.esp_flags"));
	}
	...
	setup_item(&g_cfg.esp.viewmodel_in_scope, false, crypt_str("Esp.gradient"));

	setup_item(&g_cfg.esp.new_scope_color, Color(255, 255, 255, 240), crypt_str("Menu.scope_color"));
	...
	setup_item(&g_cfg.esp.grenade_proximity_warning_inner_color, Color(255, 255, 255), crypt_str("Esp.grenade_proximity_warning_inner_color"));
	...
	setup_item(&g_cfg.misc.keybinds, false, crypt_str("Misc.key_binds"));
	...
	setup_item(&g_cfg.misc.automatic_peek_color, Color(197, 208, 230), crypt_str("Menu.automatic_peek_color"));
	...
	setup_item(&g_cfg.misc.break_lc, false, crypt_str("Misc.break_lc"));
	setup_item(&g_cfg.misc.legs_movement, 0, crypt_str("Misc.slide_walk"));
	...
	setup_item(&g_cfg.misc.fast_walk, false, crypt_str("Misc.fast_walk"));
	...
	setup_item(&g_cfg.lua.tolerance, 2, crypt_str("Ragebot.tolerance"));
	setup_item(&g_cfg.lua.override_shift, 14, crypt_str("Ragebot.override_shift"));
}
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		{
			folder = std::string(path) + crypt_str("\\Alphen\\configs\\");
			file = std::string(path) + crypt_str("\\Alphen\\configs\\") + config;
		}
