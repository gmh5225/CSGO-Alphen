...
namespace ns_render
{
	...
	void draw_rect_filled_gradient(float x, float y, float w, float h, Color color, Color color2, float gradient_type)
	{
		gradient_type = math::clamp(gradient_type, 0, 1);
		render::get().gradient((int)x, (int)y, (int)w, (int)h, color, color2, gradient_type);
	}
